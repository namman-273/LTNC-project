...
public class WatchlistController implements Initializable {
  ...
  public void handleViewDetail() {
    ...
    // FIX BUG 2: Remove listener trước khi vào BidView để BidController không bị
    // duplicate noti
    removePushListener();
    stopAutoRefresh();
  ...
  // BUG FIX 2: Dùng Set riêng để track watchedIds, tránh race condition với
  // currentData
  ...
  private void registerBalancePushListener() {
    ...
    balancePushListener = message -> {
      ...
      switch (parts[0]) {
          ...
          if (parts.length >= 4) {
            ...
            if (watchedAuctionIds.contains(auctionId)) {
              try {
                ...
                Platform.runLater(() -> ToastManager.show(ToastManager.Type.INFO,
        ...
        // FIX Bug3: RES_END_SUCCESS là direct response (1-1), KHÔNG phải broadcast
        // push.
