...
public class SellerController extends BaseController implements Initializable {
      ...
      case Protocol.NOTI_BALANCE_CHANGED:
        if (parts.length >= 4) {
          ...
          Platform.runLater(() -> {
            ...
            try {
              balanceFormatted = AuctionUtils.formatPrice(Double.parseDouble(newBalance));
            } catch (NumberFormatException e) {
  ...
  // ── History row cell (đã tồn tại, giữ nguyên, chỉ inlined vào inner class) ──
  private static class HistoryRowCell extends javafx.scene.control.ListCell<String> {
    @Override
    protected void updateItem(String item, boolean empty) {
      ...
      String[] arrow = rest.split("→", 2);
      final String bidder = arrow.length > 0 ? arrow[0].trim() : rest;
      final String price = arrow.length > 1 ? arrow[1].trim() : "";
