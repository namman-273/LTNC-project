...
public class AuctionService implements Serializable {
    ...
    // Sau khi load từ file, ta cần gán lại instance để cả hệ thống dùng chung bản vừa load
    public static void setInstance(AuctionService loadedInstance) {
        //chúng ta đang ở trong một hàm static (hàm của lớp, không phải của đối tượng cụ thể). Lúc này, chưa có đối tượng instance nào tồn tại để mà khóa, nên ta phải dùng chính cái "khuôn mẫu" của lớp đó (AuctionService.class) làm điểm tựa để khóa.
        synchronized (AuctionService.class) {
